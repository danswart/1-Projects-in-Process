...
emp_cl <<- base::mean(df1_control_limits$value, na.rm = TRUE) # Calculate empirical centerline
emp_sd <<- base::mean(
  base::abs(base::diff(df1_control_limits$value)),
  na.rm = TRUE
) /
  1.128 # Calculate empirical standard deviation
emp_lcl <<- base::max(emp_cl - (3 * emp_sd), 0) # If LCL is negative, set it to zero
...
runs <- base::sign(df1_modified$value - emp_cl)
runs <- runs[runs != 0]
...
df1_modified <- df1_modified |>
  dplyr::mutate(
    ...
    runs_signal = base::as.logical(runs_signal) # Ensure it's a column, not a constant # add runs_signal
  )
...
desired_years = base::unique(df1_modified$year)

# Create char vector of unique entries in 'group' column
desired_groups = base::unique(df1_modified$group)

# Create char vector of unique entries in 'grade_level' column
desired_grade_levels <- base::unique(df1_modified$grade_level_name)

# Create char vector of unique entries in 'subject' column
desired_subjects <- base::unique(df1_modified$subject)

# Create char vector of unique entries in 'achieve_level' column
desired_achieve_levels <- base::unique(df1_modified$staar_grouping)

# Create num vector of unique entries in 'value' column
desired_values <- base::unique(df1_modified$value)
...
df1_modified$value <- base::ifelse(
  df1_modified$value == 0,
  NA,
  df1_modified$value
)
...
x_pos <- base::min(df1_modified$year, na.rm = TRUE) # Ensure NA handling
y_pos <- base::max(df1_modified$value, na.rm = TRUE) # Ensure NA handling
...
i_subtitle <- base::paste0(
  "In ",
  desired_grade_levels,
  "<br>",
  "For the Years Ended ",
  base::paste0(base::min(desired_years), " - ", base::max(desired_years))
)
...
i_chart <- ggplot2::ggplot(
  ...
  ggplot2::geom_point(
    ggplot2::aes(
      x = year,
      y = value,
      color = base::factor(emp_pts_out_of_control) # Map to emp_pts_out_of_control to color the points
    ),
    size = 2.5
  ) +

  ggplot2::scale_color_manual(
    values = base::c("TRUE" = "red", "FALSE" = "blue")
  ) + # Define colors for dots based on value of sigma_signals

  # CL line type depends on value of runs_signals
  ggplot2::geom_line(ggplot2::aes(
    y = emp_cl,
    linetype = base::factor(runs_signal)
  )) +

  ggplot2::scale_linetype_manual(
    values = base::c("FALSE" = "solid", "TRUE" = "dashed")
  ) +
  ...
  ggtext::geom_richtext(
    ggplot2::aes(
      label = base::paste0(
        "<b><i>UPL, CL, LPL are based on the years:  ",
        base::min(df1_control_limits$year),
        " - ",
        base::max(df1_control_limits$year),
        "</i></b>"
      ),
      x = x_pos,
      y = y_pos
  ...
  ggtext::geom_richtext(
    ggplot2::aes(
      ...
      x = base::max(df1_modified$year, na.rm = TRUE), # Ensure NA handling
      y = base::min(df1_modified$value, na.rm = TRUE) # Ensure NA handling
    ),
    size = 6, # Font size (adjusted to a reasonable level)
    color = "black",
  ...
  ggplot2::scale_x_continuous(
    breaks = base::unique(df1_modified$year),
    expand = base::c(0.15, 0.00),
  ) +

  ggplot2::scale_y_continuous(
    labels = function(y) {
      base::format(y, scientific = FALSE, big.mark = ",")
    },
    name = base::unique(df1_modified$units),
    expand = base::c(0.25, 0.25) # Add padding on both sides of y-axis on i_chart
  ) +
...
i_chart <- i_chart +
  ...
  ggplot2::geom_text(
    ggplot2::aes(
      ...
      label = "CenterLine", # base::format(base::round(dplyr::first(emp_cl), 1   # dplyr function to round first value in 'centerline' column
    ),
    color = "blue",
    vjust = -1,
    hjust = 1.1,
  ...
  # ggplot2::geom_text(ggplot2::aes(x = stats::median(year),                           # stats function to set the x position of text
  #               y = stats::median(emp_cl),                     # stats function to set the y position of text
  #               label = base::format(base::round(stats::median(emp_cl), 1),   # stats function to round median value in 'centerline' column
  #                              big.mark = ","
  #                              )
  #               ),
  #           color = "blue",
  ...
  ggplot2::geom_text(
    ggplot2::aes(
      ...
      label = "Upper Process Limit", #base::format(base::round(dplyr::first(emp_ucl), 1   # dplyr function to round first value in 'ucl' column
    ),
    color = "red",
    vjust = -1,
    hjust = 1.1,
  ...
  #               label = base::format(base::round(stats::median(emp_ucl), 1    # stats function to round median value in 'ucl' column
  #                                    ),
  #                              big.mark = ","
  #                              )
  #               ),
  ...
  ggplot2::geom_text(
    ggplot2::aes(
      ...
      label = "Lower Process Limit" # base::format(base::round(dplyr::first(emp_lcl), 1     # dplyr function to round first value in 'ucl' column
    ),
    color = "red",
    vjust = 1.75,
    hjust = 1.1,
  ...
  #               label = base::format(base::round(stats::median(emp_lcl), 1    # stats function to round median value in 'lcl' column
  #                                    ),
  #                              big.mark = ","
  #                              )
  #               ),
...
i_chart <- i_chart +
  ...
  ggplot2::theme(plot.margin = ggplot2::margin(t = 0, r = 0, b = 0, l = 0)) # Adjust padding (top, right, bottom, left)
