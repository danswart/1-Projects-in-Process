...
# runs analysis
runs        <- base::sign(y - base::ifelse(x <= 15, data$centerline_first_15, data$centerline_remaining))
runs        <- runs[runs != 0]
...
ggplot2::ggplot(data, ggplot2::aes(x = x, y = y)) +
  ggplot2::geom_point(ggplot2::aes(x = x, y = y), shape = 19, size = 3, col = sigma.signals + 1) +
  ggplot2::geom_line() +

  # Draw CL, UCL & LCL for the first 15 points
  ggplot2::geom_line(ggplot2::aes(y = base::ifelse(x <= 15, centerline_first_15, NA)), color = "blue", linetype = "solid", linewidth = 1) +
  ggplot2::geom_line(ggplot2::aes(y = base::ifelse(x <= 15, ucl_first_15, NA)), color = "red", linetype = "dotted", linewidth = 1) +
  ggplot2::geom_line(ggplot2::aes(y = base::ifelse(x <= 15, lcl_first_15, NA)), color = "red", linetype = "dotted", linewidth = 1) +

  # Draw CL, UCL & LCL for the remaining points
  ggplot2::geom_line(ggplot2::aes(y = base::ifelse(x > 15, centerline_remaining, NA)), color = "green", linetype = "solid", linewidth = 1) +
  ggplot2::geom_line(ggplot2::aes(y = base::ifelse(x > 15, ucl_remaining, NA)), color = "red", linetype = "dotted", linewidth = 1) +
  ggplot2::geom_line(ggplot2::aes(y = base::ifelse(x > 15, lcl_remaining, NA)), color = "red", linetype = "dotted", linewidth = 1) +

  # Add labels
  ggplot2::labs(title = "Trended Control Chart with Recalced Limits",
