...
# Builds dropdown selections for each column (variable) found in chosen database
...
##################  renderUI FUNCTIONS UPDATE DROPDOWN CHOICES  #####
...
output$chart_title_selections <- renderUI({
  ...
  selectInput(
    ...
    selected = column_choices()[2],  # Default to first column
...
output$chart_subtitle_selections <- renderUI({
  ...
  selectInput(
    ...
    label = "Select Categories to Include in Chart Subtitle:",
    choices = column_choices(),     # these are row names of selected columns
...
output$line_chart <- renderPlot({
  req(filtered_data(), input$x_column, input$y_column, input$chart_title, 
      input$year_range, input$multi_var_group)
...
output$untrended_i_chart <- renderPlot({
  ...
  desired_years <- unique(data_modified[[input$x_column]])
...
###############  BEGIN UNTRENDED I-CHART TITLES  #####
  ...
  theme(aspect.ratio = NULL)    # Override any existing aspect ratio setting
