...
# data_wide <- readr::read_csv("data/SCUC Snapshots 1995 to 2023-WIDE.csv")
...
emp_sd <<- mean(
  abs(diff(df1_control_limits$value)),
  na.rm = TRUE
) /
...
i_subtitle <- paste0(
  ...
  "  For the Years Ended ",
  paste0(min(desired_years), " - ", max(desired_years))
)
