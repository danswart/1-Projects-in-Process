...
base::options(device = "quartz")  # macOS native graphics device
...
shiny::fileInput(inputId = "file_chosen",
          label = "Choose a CVS File",
          accept = base::c(".csv")) 


# File upload observer feeds 'column_data' to global data storage called 'column_data'
shiny::observeEvent(input$file_chosen, {
  shiny::req(input$file_chosen)

  # Read the CSV file
  data <- readr::read_csv(input$file_chosen$datapath, col_names = TRUE)
...
column_choices <- shiny::reactiveVal(NULL) 
...
output$chart_title_selected <- shiny::renderUI({
...
shiny::uiOutput("chart_title_selected")      
...
output$chart_subtitle_selected <- shiny::renderUI({
  shiny::req(column_choices())
  shiny::selectInput(
...
shiny::uiOutput("chart_subtitle_selected")   
...
output$advanced_line_group_selected <- shiny::renderUI({
  shiny::req(column_choices())
  shiny::selectizeInput(  # Using selectizeInput for better multiple selection
...
shiny::uiOutput("advanced_line_group_selected")
...
groups_title <- base::paste(input$advanced_line_group, collapse = ", ")
...
title_elements <- base::c()                    # start with an empty vector
for(col_name in input$chart_title) {     # loop thru each column name in 'chart_title'
  if(col_name %in% base::names(data)) {        # if column name is within the names found in data frame
    if(base::length(base::unique(data[[col_name]])) == 1) {     # if the string has length of one
      title_elements <- base::c(title_elements, base::as.character(base::unique(data[[col_name]])))     # put that one name in the object 'title_elements'
    } else if(base::length(base::unique(data[[col_name]])) <= 5) {     # or, if the length is < 5, put the names into the object 'title_elements'
      values <- base::paste(base::unique(data[[col_name]]), collapse=", ")
      title_elements <- base::c(title_elements, values)
    } else {     # if there are more than 5 names, just pint 'x number of values'
      n_values <- base::length(base::unique(data[[col_name]]))
      title_elements <- base::c(title_elements, base::paste0(n_values, " Values"))
    }
  }
}
...
title_content <- if (base::length(title_elements) > 0) {
  base::paste(title_elements, collapse = " - ")
} else {
  "Selected Data"
}
...
ggplot2::labs(
  title = title,
  subtitle = subtitle,
  caption = base::paste(" Comparison by ", groups_title),
  x = input$x_column,
  y = input$y_column,
...
ggplot2::theme(
  ...
  plot.title = ggtext::element_markdown(
     ...
     face = "bold",
     lineheight = 1.3,
     margin = ggplot2::margin(0, 0, 0.25, 0, "lines")
  ),
